const myGetHttp = (url) => {
  ...
  async function promiseHell() {
    const getHttp = await myGetHttp('http://api.something.com/do');
    console.log('Got HTTP response');
    
    const parser = await myParser(getHttp);
    console.log('Got parsed HTTP response');
    
    const dbSaver = await myDbSaver(parser);
    console.log('Response is saved in the DB');
    
    const notifyUser = await notifyUser(dbSaver);
